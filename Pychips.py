...
def flict():
    ...
    if tpp == '.png' or tpp == '.jpeg' or tpp == '.jpg':
        input(clr.yellow+'\nRep'+clr.end+' : Press Enter to'+clr.green+' Rendering '+clr.end+'the `'+username()+tpp+'`..')
        loading(5,'Start Rendering..')
        writer(username()+tpp,pic)
        sys(ctype[os])
        print(clr.yellow+'\nRep'+clr.end+' : Rendering has'+clr.green+' Finished!')
        slp(2)
        print(clr.yellow+'Rep'+clr.end+' : Please check the '+clr.green+'.'+clr.end+' Directory!')
        login('rendering '+username()+tpp,username())
